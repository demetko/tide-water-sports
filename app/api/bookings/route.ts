...
export async function GET(req:Request){try{
 ...
 return Response.json(await supabase('/rest/v1/bookings?booking_date=eq.'+date+'&select=id,equipment_id,customer_name,customer_phone,booking_date,start_time,duration_hours,total_price_bgn,payment_status,fulfillment_status,quantity,departure&order=start_time,id',undefined,token),{headers:{'Cache-Control':'no-store'}});
