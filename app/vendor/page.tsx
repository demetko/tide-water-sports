...
type Booking={id:number,equipment_id:number,customer_name:string,customer_phone:string,booking_date:string,start_time:string,duration_hours:number,total_price_eur:number,payment_status:Key,fulfillment_status:Key,quantity:number,departure:string};
export default function Vendor(){
 ...
 <div className="metrics"><div><span>{t('bookings')}</span><strong>{hasData?bookings.length:'—'}</strong><CalendarDays/></div><div><span>{t('active')}</span><strong>{hasData?bookings.filter(b=>b.fulfillment_status==='Active').reduce((s,b)=>s+b.quantity,0):'—'}</strong><Waves/></div><div><span>{t('revenue')}</span><strong>{hasData?money(paid.reduce((s,b)=>s+Number(b.total_price_eur),0)):'—'}</strong><Anchor/></div></div>
