...
export default function Home(){
 ...
 const total=e.hourly_rate_bgn*duration*quantity,now=coastNow(),past=date<now.date||(date===now.date&&hour<=now.hour);
 ...
 useEffect(()=>{
  ...
  Promise.all([api('/api/equipment',{signal:ctrl.signal}),api('/api/availability?date='+date+'&hour='+hour+'&duration='+duration,{signal:ctrl.signal})])
   .then(([equipment,availability])=>{
    ...
    if(!Array.isArray(equipment)||equipment.length!==3||!Array.isArray(availability)||availability.length!==3)throw new Error('UNAVAILABLE');
 ...
 async function pay(event:React.FormEvent<HTMLFormElement>){
  ...
  const payload={equipment:e.id,name:form.get('name'),phone:form.get('phone'),date,hour,duration,quantity,departure,amount:total,terms:true,payment_token:'pm_mock_visa'};
 ...
 const money=(v:number)=>new Intl.NumberFormat(lang,{maximumFractionDigits:2}).format(v);
 return <><main className="shell">
  ...
  <div className="workspace"><section><div className="section-heading"><h2>{t('choose')}</h2><span>3 {t('experiences')}</span></div><div className="catalog">
   {items.map((item,i)=><button type="button" aria-pressed={selected===i} onClick={()=>choose(i)} key={item.id} className={'experience '+(selected===i?'selected':'')}>
    ...
    <div className="card-content"><h3>{t(titleKeys[i])}</h3><p>{i===1?t('paraType'):item.model}</p><div className="card-detail"><Clock3 size={14}/><span>1–4 {t('hours')}</span><span>·</span><span>{t('briefing')}</span></div><div className="card-bottom"><strong>{money(item.hourly_rate_bgn)} <small>BGN / {t('hour')}</small></strong><span className="availability">{ready?counts[item.id]:item.max_quantity} {t(ready?'available':'capacity')}</span></div></div>
   </button>)}
  </div><div className="assurances"><div><ShieldCheck/><span><strong>{t('peace')}</strong><small>{t('safety')}</small></span></div><div><Anchor/><span><strong>{t('people')}</strong><small>{t('operators')}</small></span></div><div><Waves/><span><strong>{t('weather')}</strong><small>{t('refund')}</small></span></div></div></section>
  <aside className="booking-panel"><div className="panel-title"><h2>{t('yourTime')}</h2><span>01 / 02</span></div><p className="chosen">{t(titleKeys[selected])}</p>
   ...
   <div className="price-row"><span>{quantity} × {money(e.hourly_rate_bgn)} BGN × {duration} {t(duration===1?'hour':'hours')}</span><strong>{money(total)} BGN</strong></div>
   <div className="price-row included"><span>{t('briefing')}</span><strong>{t('included')}</strong></div>
   <div className="total"><span>{t('total')}</span><strong>{money(total)}<small> BGN</small></strong></div>
 ...
 <Dialog open={open} onOpenChange={v=>{if(!busy)setOpen(v)}}><DialogContent showCloseButton={false} className="checkout-modal">
  ...
  {result?<div className="success-content"><span className="success-icon"><Check size={32}/></span><h3>{t(titleKeys[selected])}</h3><p>{date} · {String(hour).padStart(2,'0')}:00 · {t(locationKeys[departures.indexOf(departure)])}</p><p>{quantity} × {duration} {t('hours')} · {money(result.total)} BGN · {t('Paid')}</p><div className="reference"><small>{t('confirmation')}</small><code>{result.confirmation}</code></div><p className="slot-note">{t('test')}</p><button className="primary" onClick={()=>setOpen(false)}>{t('done')}</button></div>:
  <form onSubmit={pay}><div className="checkout-summary"><strong>{t(titleKeys[selected])}</strong><span>{date} · {String(hour).padStart(2,'0')}:00 · {quantity} × {duration} {t('hours')}</span><b>{money(total)} BGN</b></div>
   ...
   <button className="primary" type="submit" disabled={busy||!terms}>{busy?t('paying'):t('pay')+' · '+money(total)+' BGN'}<LockKeyhole size={15}/></button>
  </form>}
